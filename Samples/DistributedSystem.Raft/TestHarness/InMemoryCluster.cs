...
public static class InMemoryCluster
{
    public static async Task DemoAsync()
    {
        ...
        var storage = new FilePersistentStorage(dataFolder);
        var network = new InMemoryRaftNetwork(10);
        ...
        foreach (var id in ids)
        {
            ...
            var node = new RaftNode(id, peers, network, storage);
            nodes.Add(node);
        ...
        for (int i = 0; i < 40; i++)
        {
            leader = nodes.FirstOrDefault(n => n.State == NodeState.Leader);
